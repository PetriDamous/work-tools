$(document)
    .ready(function() {       
        ...
        //Master Slide Tool Tips
        $('p:contains("Pause")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id")
                }
                btnPause = $(thisObjPar);
                thisPause = thisObjIDPar;
                $(btnPause).prop("title", "Pause");
                $(this).parent().remove();
            }
        )

        $('div:contains("Pause")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Play")').each(
            function(idx_a) {
                ...
                btnPlay = $(thisObjPar);
                thisPlay = thisObjIDPar;
                $(btnPlay).prop("title", "Play");
                $(this).parent().remove();
            }
        )

        $('div:contains("Play")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Closed Caption")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id")
                    $("#" + thisObjIDPar).prop("title", "Closed Caption Open")
                }
        ...
        $('div:contains("Closed Caption")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Mute")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id")
                    $("#" + thisObjIDPar).prop("title", "Mute Audio")
                }

                btnMute = thisObjPar;
                thisMute = thisObjIDPar;
                $(this).parent().remove();
            }
        )

        $('div:contains("Mute")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Unmute")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id")
                    $("#" + thisObjIDPar).prop("title", "Unmute Audio")
                }
                btnUnmute = thisObjPar;
                thisUnmute = thisObjIDPar;

                $(this).parent().remove();
            }
        )

        $('div:contains("Unmute")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Replay")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id")
                    $("#" + thisObjIDPar).prop("title", "Replay")
                }
                btnReplay = $(thisObjPar);                
                thisReplay = thisObjIDPar;                
                $(this).parent().remove();
            }
        )

        $('div:contains("Replay")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Rewind")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id")
                    $("#" + thisObjIDPar).prop("title", "Rewind")
                }
                btnRewind = $(thisObjPar);                
                thisRewind = thisObjIDPar;                
                $(this).parent().remove();
            }
        )

        $('div:contains("Rewind")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Previous")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id")
                    $("#" + thisObjIDPar).prop("title", "Previous")
                }
                btnPrev = $(thisObjPar);            
                thisPrev = thisObjIDPar;               
                $(this).parent().remove();
            }
        )

        $('div:contains("Previous")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
                }
                btnPrevious = thisObjPar;
                thisPrevious = thisObjIDPar;
            }
        )

        $('p:contains("Next")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id")
                    $("#" + thisObjIDPar).prop("title", "Next")
                }
                btnNext = $(thisObjPar);                
                thisNext = thisObjIDPar;               
                $(this).parent().remove();
            }
        )

        $('div:contains("Next")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Menu")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this).parent().parent();
                    var thisObjIDPar = $(thisObjPar).attr("id");
                    $("#" + thisObjIDPar).prop("title", "Menu");                                       
                }
                btnMenu = $(thisObjPar);
                thisMenu = thisObjIDPar;
                $(this).parent().remove();
            }
        )
        
        $('div:contains("Menu")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Help")').each(
            function(idx_a) {
                ...
                btnHelp = $(thisObjPar);
                thisHelp = thisObjIDPar;
                $(this).parent().remove();
            }
        )

        $('div:contains("Help")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Options")').each(
            function(idx_a) {
                ...
                btnOptions = $(thisObjPar);
                thisOptions = thisObjIDPar;
                $(this).parent().remove();
            }
        )

        $('div:contains("Options")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $('p:contains("Exit")').each(
            function(idx_a) {
                ...
                btnExit = $(thisObjPar);
                thisExit = thisObjIDPar;
                $(this).parent().remove();
            }
        )

        $('div:contains("Exit")').each(
            function(idx_a) {
                if (idx_a === 0) {
                    var thisObjPar = $(this);
                    var thisObjIDPar = $(thisObjPar).attr("id")
        ...
        $(btnPlay).click(function(){
            //Resumes play
            function resumePlay(){cpCmndResume = true;} 
